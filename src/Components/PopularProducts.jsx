...
const Popuarproducts = () => {
  ...
  return (
    <div className="mx-5 w-calc(100% - 32px)">
      ...
      <div className="grid w-full aa my-5 gap-4 md:grid-cols-5 smal:grid-cols-2 sm:grid-cols-3 smal:justify-between justify-center justify-items-center items-center overflow-scroll">
        {products.map((product, index) => (
          <Link to={`/product/${product.id}`}><div key={index} className="flex w-full pb-2 flex-col bg-white drop-shadow-lg border-y-[1px] rounded-sm gap-2">
            <img
