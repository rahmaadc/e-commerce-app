        ...
        <div className="flex w-full flex-row justify-between sm:gap-8 gap-1 items-center">
          ...
          <form onSubmit={()=>{
            ...
            <input
              ...
              className="border-[1px] border-black rounded-2xl ps-4 py-1 w-full h-12 focus:outline-none focus:border-[1.5px]"
            />
            {/* S'inscrier button */}
          </form>
          {isAuth === false ? (
            <Link
              to="/login"
              className="bg-[#D9D9D9] text-black text-[11px] gg:text-md sm:px-10 px-[20px] py-3 rounded-2xl"
            >
              S'inscrire
            </Link>
          ) : (
